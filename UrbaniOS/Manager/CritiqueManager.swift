...
class CritiqueManager {
    ...
    func createCritique(title: String, descriptionHTML: String, mediaUrls: [UploadedMedia], defaultMediaUrl: UploadedMedia, completionHandler: @escaping (Critique?) -> ()) {
        ...
        let medias: [MediaCreateInput] = mediaUrls.enumerated().compactMap({ MediaCreateInput(srcUrl: $1.url, position: $0) })
        
        let input: CritiqueCreateInput = .init(title: title, descriptionHtml: descriptionHTML, style: .review, mediaUrls: medias, defaultMediaUrl: defaultMediaUrl.url, tags: tags, categories: [])
